...
Public Class StaffRecord_New
        ...
        If TxtFName.Text <> "" And TxtLName.Text <> "" And NumY.Text <> "" And NumM.Text <> "" And NumD.Text <> "" And TxtICPass.Text <> "" And TxtAddress1.Text <> "" And TxtAddress2.Text <> "" And TxtMNumber.Text <> "" And TxtEmail.Text <> "" And CBoxPos.Text <> "" And CBoxAvai.Text <> "" And TxtPass.Text <> "" Then
            ...
            If IsNumeric(NumY.Text) = True And IsNumeric(NumM.Text) = True And IsNumeric(NumD.Text) = True Then
                Try
                    ...
                    'Increment primary key, refer function below
                    IncrementID()
                    ...
                    'SQL Command
                    cmdOLEDB.CommandText = "INSERT INTO [Staff] VALUES ('" + StoreID + "','" + TxtFName.Text + "','" + TxtLName.Text + "'," + NumY.Text + "," + NumM.Text + "," + NumD.Text + ",'" + TxtICPass.Text + "','" + TxtAddress1.Text + "','" + TxtAddress2.Text + "','" + TxtMNumber.Text + "','" + TxtEmail.Text + "','" + CBoxPos.Text + "','" + CBoxAvai.Text + "','" + TxtPass.Text + "');"
    ...
    Private Sub StaffRecord_New_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    'CLOSE FORM EVENT
    Private Sub StaffRecord_New_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Sub IncrementID()
        ...
        If CBoxPos.Text = "Administrator" Then
            'SQL Command
            cmdOLEDB.CommandText = "SELECT MAX (StaffID) FROM [Staff] WHERE [StaffID] LIKE 'A%';"
            cmdOLEDB.CommandType = CommandType.Text
            cmdOLEDB.Connection = cnnOLEDB
            ...
            IDNum += 1
            StoreID = "A" & IDNum.ToString("D3")

            cnnOLEDB.Close()
        ElseIf CBoxPos.Text = "Doctor" Then
            'SQL Command
            cmdOLEDB.CommandText = "SELECT MAX (StaffID) FROM [Staff] WHERE [StaffID] LIKE 'D%';"
            cmdOLEDB.CommandType = CommandType.Text
            cmdOLEDB.Connection = cnnOLEDB
            ...
            IDNum = CInt(StoreID.Substring(1))
            IDNum += 1
            StoreID = "D" & IDNum.ToString("D3")

            cnnOLEDB.Close()
        ElseIf CBoxPos.Text = "Nurse" Then
            'SQL Command
            cmdOLEDB.CommandText = "SELECT MAX (StaffID) FROM [Staff] WHERE [StaffID] LIKE 'N%';"
            cmdOLEDB.CommandType = CommandType.Text
            cmdOLEDB.Connection = cnnOLEDB
